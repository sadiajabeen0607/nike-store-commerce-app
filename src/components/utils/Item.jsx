...
const Item = ({ifExists, id, color, shadow, title, text, img, btn, rating, price }) => {
  ...
  const onAddToCart = () => {
    const item = { id, title, color, text, img, price, shadow }
  ...
  return (
    <>
      <div
        className={`relative bg-gradient-to-b ${color} ${shadow} grid items-center ${ifExists? 'justify-items-start' : 'justify-items-center'} rounded px-5 py-4 transition-all duration-700 ease-in-out w-full hover:scale-105`}
      >
        <div className={`grid items-center ${ifExists? 'justify-items-start' : 'justify-items-center'}`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <button type="button" className="bg-white/90 blur-effect-theme button-theme px-2 py-1 shadow shadow-sky-200 text-sm text-black"
